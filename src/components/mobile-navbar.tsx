...
import { Sun, Moon, Home, Info, Boxes, Calendar, MessageSquare, Leaf, Building2 } from 'lucide-react'
        ...
        <>
            ...
            {showFloatingNav && (
                <div className={`fixed top-3 left-0 w-full flex justify-center z-50 sm:hidden pointer-events-none transition-all duration-300 ${condensed ? 'py-1' : ''}` }>
                    <div className={`pointer-events-auto flex items-center justify-between gap-2 ${condensed ? 'px-3 py-1' : 'px-6 py-2'} rounded-2xl min-w-[45vw] bg-white/95 dark:bg-black/90 shadow-2xl backdrop-blur-lg border border-white/30 dark:border-black/30 w-fit mx-auto transition-all duration-300 sm:hidden`} style={{background: 'linear-gradient(90deg, #fff 80%, #f3f4f6 100%)'}}>
                        ...
                        <div className="flex-1 flex justify-center">
                            {mounted && (
                                <button
                                    ...
                                    className="rounded-full p-2 bg-gray-100 dark:bg-black focus:outline-none border border-gray-200 dark:border-gray-700 shadow-md"
            ...
            {/* Top mobile header: logo and theme toggle only */}
            {/* REMOVE the nav with className 'lg:hidden fixed top-0 ...' and all its contents */}
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="sm:hidden absolute top-full left-0 w-full bg-white dark:bg-black border-b border-gray-200 dark:border-gray-800 shadow-md"
                    >
            ...
            <nav className="fixed bottom-3 left-0 w-full flex justify-center z-50 sm:hidden pointer-events-none">
                <div className="pointer-events-auto flex w-[95vw] max-w-md justify-between items-center px-2 py-1.5 rounded-2xl bg-white/80 dark:bg-black/70 shadow-xl backdrop-blur-md border border-white/20 dark:border-black/20">
                    {menuItems.map((item) => {
